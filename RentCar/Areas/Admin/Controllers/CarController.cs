    ...
    public class CarController : Controller
    {
        ...
        public async Task<IActionResult> Create(CarCreateViewModel model)
        {
            ...
            for (int i = 0; i < model.Files.Length; i++)
            {
                ...
                if (!model.Files[i].IsSupported())
                {
                    ModelState.AddModelError("Files", "File is unsupported");
                    return View();
                }
                if (model.Files[i].IsGreaterThanGivenSize(1024))
                {
                    ModelState.AddModelError(nameof(CarCreateViewModel.Files),
                        "File size cannot be greater than 1 mb");
                    return View();
                ...
                var imgName = FileUtil.CreatedFile(Path.Combine(FileConstants.ImagePath, "cars"), model.Files[i]);
                carImages.Add(new CarImage { ImageName = imgName, IsMain = IsMainPhoto });
            }
        ...
        public async Task<IActionResult> Update(int id, CarUpdateViewModel model)
        {
            ...
            if (model.MileAge != 0)
            {
                updatedCarModel.MileAge = model.MileAge;
            }
